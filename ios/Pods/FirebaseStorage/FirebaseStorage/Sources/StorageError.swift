...
@objc(FIRStorageErrorCode) public enum StorageErrorCode: Int, Swift.Error {
    ...
    static func error(withServerError serverError: NSError, ref: StorageReference) -> NSError {
        ...
        case 400:
            storageError = .unknown(message: "Unknown 400 error from backend", serverError: errorDictionary)
        case 401:
            storageError = .unauthenticated(serverError: errorDictionary)
        case 402:
            storageError = .quotaExceeded(bucket: ref.path.bucket, serverError: errorDictionary)
        case 403:
        ...
        default:
            storageError = .unknown(message: "Unexpected \(serverError.code) code from backend", serverError: errorDictionary)
        }
...
public enum StorageError: Error, CustomNSError {
    ...
    public var errorUserInfo: [String: Any] {
        ...
        case let .downloadSizeExceeded(total, maxSize):
            ...
            dictionary[NSLocalizedDescriptionKey] = "Attempted to download object with size of \(total) bytes, " +
